                ...
                @if(auth('customer')->user())
                 @if(auth('customer')->user()->ranking == 1)
                    @php
                     ...
                     @endphp
                    <h5>Discount ({{auth('customer')->user()->subcontractor_dis}}%):<strong>{{ format_price($dis) }}</strong></h5>
                 @endif
                 @endif
...
{{--                <h3>{{ __('Total') }}:<strong>{{ format_price(Cart::instance('cart')->rawSubTotal() + Cart::instance('cart')->rawTax())}}</strong></h3>--}}
                <h3>{{ __('Total') }}:<strong>{{ format_price(Cart::instance('cart')->rawTotal())}}</strong></h3>
            @else
