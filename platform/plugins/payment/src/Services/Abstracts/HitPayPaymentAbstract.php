...
require_once __DIR__.'/HitPaySdk/vendor/autoload.php';

abstract class HitPayPaymentAbstract implements ProduceServiceInterface
{
    ...
    {
        $apiKey = setting('payment_hitpay_api_key', '<<HITPAY-API-KEY>>');
        $this->client = new \HitPay\Client($apiKey, false);
        return $this;
    }
    ...
    {
        $this->totalAmount = round((float) $amount, 2);;
        return $this;
    }
...
//        echo($this->returnUrl);exit();
        try {
            ...
            $request->setAmount((string)$this->totalAmount)
                ->setCurrency($this->paymentCurrency)
                ->setRedirectUrl($this->returnUrl);
...
//            echo('12313');exit;
        ...
        if ($checkoutUrl && $paymentId) {
            session(['hitpay_payment_id' => $paymentId]);
            return $checkoutUrl;
        }
...
//        }
        ...
        $paymentId = session('hitpay_payment_id');
//        exit($paymentId);
        try {
            $result = $this->client->getPaymentStatus($paymentId);
//            print_r($result);exit;
            if ($result && $result->status == 'completed') {
                return $result->status;
            }else{
                $response->setNextUrl($result->redirect_url);
            }
        } catch (Exception $exception) {
            $this->setErrorMessageAndLogging($exception, 1);
        }
    ...
    {
        try {
            $response = $this->client->getPaymentStatus($paymentId);
